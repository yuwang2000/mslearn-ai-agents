...
Tip: The code used in this exercise is based on the for Foundry SDK for Python. You can develop similar solutions using the SDKs for Microsoft .NET, JavaScript, and Java. Refer to Foundry SDK client libraries for details.
...
In a web browser, open the Foundry portal at https://ai.azure.com and sign in using the username User1-61903693@LODSPRODMCA.onmicrosoft.com and the TAP 7UBjzQW6. Close any tips or quick start panes that are opened the first time you sign in, and if necessary use the Foundry logo at the top left to navigate to the home page, which looks similar to the following image (close the Help pane if it's open):
...
When prompted to create a project, enter Project61903693 and expand Advanced options.

Confirm the following settings for your project:

Foundry resource: Project61903693-resource
...
Screenshot of a Foundry project overview page.

Copy the Foundry project endpoint values to a notepad, as you'll use them to connect to your project in a client application.
...
Open a new browser tab (keeping the Foundry portal open in the existing tab). Then in the new tab, browse to the Azure portal at https://portal.azure.com; signing in with the username User1-61903693@LODSPRODMCA.onmicrosoft.com and the TAP 7UBjzQW6 if prompted.
...
In the code file, replace the your_project_endpoint placeholder with the endpoint for your project (copied from the project Overview page in the Foundry portal), and the your_model_deployment placeholder with the name you assigned to your gpt-4o model deployment (which by default is gpt-4o).
...
Enter the following command to edit the agent_triage.py file:
...
Note that code to load the project endpoint and model name from your environment variables has been provided.
...
Find the comment Create an agent to estimate effort for a support ticket, and enter the following code:
...
Find the comment Create connected agent tools for the support agents, and enter the following code:
...
Find the comment Create an agent to triage support ticket processing by using connected agents, and enter the following code:
...
Find the comment Use the agents to triage a support issue, and enter the following code:
...
Find the comment Clean up, and enter the following code to delete the agents when they are no longer required:
...
When prompted, follow the instructions to open the sign-in page in a new tab and enter the authentication code provided and the username User1-61903693@LODSPRODMCA.onmicrosoft.com and the TAP 7UBjzQW6. Then complete the sign in process in the command line, selecting the subscription containing your Foundry hub if prompted.

After you have signed in, enter the following command to run the application:

TypeCopy
python agent_triage.py
Enter a prompt, such as Users can't reset their password from the mobile app.
